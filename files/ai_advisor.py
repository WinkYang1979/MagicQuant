...
输出严格 JSON(不要任何其他文字):
{
  ...
  "qty":        整数股数(HOLD 时为 0),
  "price":      建议成交价(当前价附近, HOLD 时为 null),
...
老杨用 Moomoo AU 做 RKLB 系列波段做 T.你看到:
  ...
  2. 当前市场数据
  3. 老杨的持仓
  4. 3 位 AI 顾问(Haiku, DeepSeek, GPT-5)的独立建议

你的任务:
  1. 对比 3 位顾问的意见,识别共识与分歧
  2. 结合原始数据做独立判断
  3. 给出最终建议,可以 BUY/SELL/HOLD
  4. 同意跟随多数顾问意见,但若你有更强的判断依据,应独立决策
  5. **如果顾问分歧大或信号弱,建议 HOLD 更安全**

输出严格 JSON(不要任何其他文字):
{
  "final_action":   "BUY" | "SELL" | "HOLD",
  "final_ticker":   "RKLB" | "RKLX" | "RKLZ" | null,
  "final_qty":      整数股数,
  "final_price":    建议成交价(当前价附近, HOLD 时为 null),
  "final_reason":   "2-3 句话中文总结,最多 100 字,说明主要依据",
...
def _build_leader_prompt(
    ...
    lines += [
        "",
        "请综合以上 3 位顾问的意见和原始数据,给出最终建议 (JSON).",
