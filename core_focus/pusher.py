...
def _calc_price_targets(session, ticker: str, direction: str, entry_price: float) -> dict:
    ...
    方法 B（历史统计）:
      - 读 data/review/ 历史 triggers.json
      - 统计同类信号触发后 30 分钟内实际涨跌幅
      - 取中位数作为预期目标
    ...
    try:
        ...
        if len(prices_ts) >= 10:
            recent = [p for _, p in prices_ts[-60:]]  # 近 60 分钟价格
            recent_high = max(recent)
            recent_low  = min(recent)

            # ATR 估算（用近 20 根价格的标准差 × 1.5）
            import statistics
            if len(recent) >= 20:
                std = statistics.stdev(recent[-20:])
                atr_est = std * 1.5
            else:
                atr_est = entry_price * 0.005  # 默认 0.5%

            if direction == "long":
                result["tech_target"] = round(min(recent_high, entry_price + atr_est * 2), 2)
                result["tech_stop"]   = round(max(recent_low,  entry_price - atr_est * 1.5), 2)
            else:
                result["tech_target"] = round(max(recent_low,  entry_price - atr_est * 2), 2)
                result["tech_stop"]   = round(min(recent_high, entry_price + atr_est * 1.5), 2)
    except Exception as e:
        print(f"  [pusher] tech target calc error: {e}")
    ...
    try:
        ...
        gains = []  # 方向正确时的涨跌幅
        stops = []  # 方向错误时的涨跌幅（取绝对值）

        if os.path.exists(review_dir):
            # 读最近 7 天的 triggers.json
            dates = sorted(os.listdir(review_dir), reverse=True)[:7]
            for date_dir in dates:
                tf = os.path.join(review_dir, date_dir, "triggers.json")
                ...
                try:
                    ...
                    for rec in records:
                        if rec.get("direction") != direction:
                            continue
                        prices_snap = rec.get("prices", {})
                        tk_short = ticker.replace("US.", "")
                        rec_price = prices_snap.get(tk_short, 0)
                        if not rec_price:
                            continue
                        # 这里只有触发时价格，没有30分钟后的价格
                        # 用当天高低点估算（如果 session_high/low 在 summary 里）
                        gains.append(rec_price)  # 占位，后续可接 K 线数据精算
                except Exception:
                    pass

        if len(gains) >= 5:
            import statistics
            avg_price = statistics.mean(gains)
            # 粗估：历史同类信号触发后价格的平均偏移
            # 这里用标准差作为参考目标距离
            std_price = statistics.stdev(gains) if len(gains) > 1 else avg_price * 0.01
            offset = std_price * 0.5  # 保守估计

            if direction == "long":
                result["stat_target"] = round(entry_price + offset, 2)
                result["stat_stop"]   = round(entry_price - offset * 0.7, 2)
            else:
                result["stat_target"] = round(entry_price - offset, 2)
                result["stat_stop"]   = round(entry_price + offset * 0.7, 2)
            result["stat_samples"] = len(gains)
    except Exception as e:
        print(f"  [pusher] stat target calc error: {e}")

    # ── 冲突检测 ─────────────────────────────────────────────
    if result["tech_target"] and result["stat_target"]:
        tech_dir = result["tech_target"] > entry_price
        stat_dir = result["stat_target"] > entry_price
        result["conflict"] = (tech_dir != stat_dir)
...
def _fmt_price_targets(targets: dict, direction: str, entry_price: float) -> str:
    ...
    tech_t = targets.get("tech_target")
    tech_s = targets.get("tech_stop")
    stat_t = targets.get("stat_target")
    stat_s = targets.get("stat_stop")
    n      = targets.get("stat_samples", 0)
    conflict = targets.get("conflict", False)
    ...
    # 技术位
    if tech_t and tech_s:
        tech_pct_t = (tech_t - entry_price) / entry_price * 100
        tech_pct_s = (tech_s - entry_price) / entry_price * 100
        lines.append(
            f"🔧 技术位  {target_label} ${tech_t:.2f} ({tech_pct_t:+.1f}%)"
            f"  {stop_label} ${tech_s:.2f} ({tech_pct_s:+.1f}%)"
        )

    # 历史统计
    if stat_t and stat_s and n >= 3:
        stat_pct_t = (stat_t - entry_price) / entry_price * 100
        stat_pct_s = (stat_s - entry_price) / entry_price * 100
        lines.append(
            f"📊 历史统计({n}样本)  {target_label} ${stat_t:.2f} ({stat_pct_t:+.1f}%)"
            f"  {stop_label} ${stat_s:.2f} ({stat_pct_s:+.1f}%)"
        )
    elif n < 3:
        lines.append(f"📊 历史统计  样本不足({n}条)，仅供参考")

    # 冲突提示
    if conflict:
        lines.append(f"⚡ <b>注意</b>: 技术位与历史统计方向冲突，建议 /ask RKLB 查看 AI 判断")
...
def _fmt_signal_with_conflict(hit, session, signal_direction, title_line, tech_line):
    ...
    if entry_price:
        ...
        target_lines = _fmt_price_targets(targets, signal_direction, entry_price)
        if target_lines:
