...
Polygon.io 免费 API，拉取 RKLB 过去 730 天、RKLX/RKLZ 维持现有 1 分钟 K 线
...
# RKLB 拉 730 天（2年）；RKLX/RKLZ 上市时间短，维持现有缓存不重拉
DAYS_BACK = {"RKLB": 730, "RKLX": 365, "RKLZ": 365}
...
def fetch_all() -> dict:
    ...
    for ticker in TICKERS:
        ...
        days = DAYS_BACK.get(ticker, 365)

        if cache_path.exists():
            mtime = datetime.fromtimestamp(cache_path.stat().st_mtime)
            age_hours = (datetime.now() - mtime).total_seconds() / 3600
            # RKLX/RKLZ 始终使用缓存（上市时间有限，重拉无意义）
            # RKLB 仅当 6 小时内拉过才用缓存，否则重拉 730 天
            use_cache = (ticker != "RKLB") or (age_hours < 6)
            if use_cache:
        ...
        df = fetch_polygon_1m(ticker, days)
        if not df.empty:
...
def generate_report(ev: pd.DataFrame, ticker: str = "RKLB") -> str:
    ...
    lines = [
        ...
        f"> 策略: swing_detector v0.5.5  |  数据: RKLB 730天",
        f"",
        f"## 总体胜率",
...
def main():
    ...
    print("=" * 60)
    print("  MagicQuant 历史回测  (RKLB 730天, swing_detector v0.5.5)")
    print("=" * 60)
